            ...
            <div style="display:flex;flex-direction:column;gap:8px;">
                <template x-for="(opt, idx) in options" :key="idx">
                    <label style="display:flex;align-items:center;gap:10px;padding:10px 12px;background:#f9f9f9;border:1px solid #eee;border-radius:4px;cursor:pointer;font-size:11.5px;font-weight:600;color:#333;transition:all 0.2s;"
                           :style="activeOption === idx ? 'border-color:#e60000;background:#fff8f8;color:#e60000;' : ''"
                           @click="activeOption = idx">
                        <input type="radio" name="poll_option" :value="idx" x-model="activeOption" style="accent-color:#e60000;cursor:pointer;">
                ...
                <button type="button" 
                        ...
                        @click="submitVote"
                        style="width:100%;background:#e60000;color:#fff;font-size:10px;font-weight:900;text-transform:uppercase;letter-spacing:0.1em;padding:10px;border:none;cursor:pointer;margin-top:10px;display:flex;align-items:center;justify-content:center;gap:6px;transition:all 0.2s;"
                        :style="activeOption === null ? 'opacity:0.5;cursor:not-allowed;' : ''"
                        onmouseover="if(this.style.opacity !== '0.5') this.style.background='#c00'" 
                        onmouseout="if(this.style.opacity !== '0.5') this.style.background='#e60000'">
                ...
                <template x-if="message">
                    <p style="font-size:10px;color:#e60000;font-weight:bold;margin-top:5px;" x-text="message"></p>
                </template>
            </div>
        </template>
                ...
                <template x-for="(opt, idx) in options" :key="idx">
                    <div>
                        <div style="display:flex;justify-content:space-between;font-size:10.5px;font-weight:700;color:#333;margin-bottom:4px;">
                            <span x-text="opt.text"></span>
                            <span style="color:#e60000;" x-text="opt.percentage + '%'"></span>
                        </div>
                        <div style="background:#eee;height:6px;border-radius:3px;overflow:hidden;">
                            <div style="background:linear-gradient(to right, #e60000, #ff4d4d);height:100%;border-radius:3px;transition:width 1s cubic-bezier(0.4, 0, 0.2, 1);"
